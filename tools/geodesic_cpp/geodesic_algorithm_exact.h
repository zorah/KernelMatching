															   ...
															   IntervalWithStop* one)			//intersecting two intervals with up to three intervals in the end
{
	...
	for(short i=0; i<Ninter; ++i)							//for all points of intersection
	{
		double x = inter[i];
	...
	MapType mid_map[3];
	for(short i=0; i<Ngood_start-1; ++i)
	{
									   ...
									   std::vector<SurfacePoint>* stop_points)
{
	...
	while(!m_queue.empty())
	{
		m_queue_max_size = (unsigned int) std::max((unsigned int) m_queue.size(),m_queue_max_size);
